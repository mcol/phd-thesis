...
\left[ \begin{array}{c}
  ...
  \end{array} \right],
\]
where $p=b-Ax$, $q=c-A^Ty-s$ and $t=\mu e-XSe$, they ask what 
choice of right-hand side $t$ would produce a good search direction.

Let $w^*=(x^*,y^*,s^*)$ be an optimal primal--dual solution to 
the system above. Therefore $\Delta w^*= w^*-w$ satisfies the 
system when $t=X\Delta s^* + S\Delta x^*=t^*$. In such a case, 
...
\fb{
Jansen \cite{JansenRoosTerlaky96} proposed to use the ratio 
...
\fb{
Monteiro, Adler and Resende (citation 188 in Jansen) talk about
corrector steps.
}