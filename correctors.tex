...
The example shows that for certain starting points the corrector 
...
%  {\bf Where does this misbehaviour come from?}
The solution outlined by Cartis in \cite{Cartis04}, and then further developed 
in \cite{Cartis05}, is to reduce the influence exerted by the corrector 
by weighting it by the square of the stepsize. 
%  Besides ensuring that the corrector is really considered 
%  a second-order term as in a rigorous Taylor expansion, 
%  this proposal allows for better convergence results ({\bf which?}).
...
(\ref{MehrotraRhs}) by the allowed stepsize for the affine-scaling direction.
...
In Section~\ref{ss:MultipleCC} we have seen that the target point 
in the multiple centrality correctors technique depends on a parameter 
...
a mechanism to adaptively change these correctors in order 
...
\item[Repeat] for $k=0,1,2,\ldots$ until some convergence criteria are met:
  ...
  \item Solve system (\ref{eq:NewtonSystem}) with right-hand side $r_1$
        (\ref{PredictorRhs}) for a predictor direction $\Delta_a$.

  \item Set $\mu$ according to (\ref{Mu}) and find Mehrothra's corrector 
        direction $\Delta_c$ by solving system (\ref{eq:NewtonSystem}) 
        with right-hand side (\ref{MehrotraRhs}).
  ...
  \item Set $\Delta_p = \Delta_a +\hat\omega\Delta_c$. 
	...
	$(0,0,t)$, $t$ given by (\ref{Target}) for a centrality corrector
	direction $\Delta_m$.
...
%
In our implementation we generate a sequence of multiple centrality correctors, and for each of them we choose the optimal weight $\hat \omega$ which
maximizes the stepsizes in primal and dual spaces for a combined direction 
of form
\[
...
direction gives 
\[
(X + \alpha_P \Delta X) (S + \alpha_D \Delta S) e \;=\; XSe + \alpha_P S \Delta Xe + \alpha_D X \Delta Se + \alpha_P \alpha_D \Delta X \Delta S e,
\]
...
In our crude linesearch procedure we choose 9 points uniformly distributed 
in the interval $[\alpha_P\alpha_D, 1]$ 
...
two different iterations of problem {\tt capri}.
...
that PCx allows the use of Gondzio's multiple centrality correctors only in 4 
problems: {\tt dfl001}, {\tt maros-r7}, {\tt pds-10} and {\tt pds-20}.
%
...
In the tables of results presented in \cite{MehrotraLi}, the best performance
in terms of iteration count is obtained by PCx4, which uses 4 Krylov subspace 
vectors. These directions are combined with an affine-scaling predictor 
direction and Mehrotra's second-order correction, leading to 6 
backsolves per iteration. 
...
improve the stepsize. Problem {\tt afiro} solved by HO-4 needs 24 backsolves, 
22 of which compute different components of directions, hence the average 
number of backsolves per iteration is only 22/6 and is much smaller 
than 6. Occasionally,
...
\endhead
\hline
\multicolumn{7}{c}{\raisebox{-1ex}{Table~\ref{TimeBN}: Time comparison on other large problems (times are in seconds).}}
\endfoot
\label{TimeBN}
...
correctors \cite{Gondzio96} and added a new degree of freedom to it. Instead 
of computing the corrected direction from $\Delta = \Delta_p + \Delta_c$
where $\Delta_p$ and $\Delta_c$ are the predictor and corrector terms, 
we allow a choice of weight $\omega \in (0,1]$ for the corrector term 
and compute $\Delta = \Delta_p + \omega \Delta_c$.
...
including medium to large scale linear programs beyond the Netlib collection
and medium scale quadratic programs.
