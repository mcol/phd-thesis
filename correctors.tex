...
starting points the corrector 
is always orders of magnitude larger than the predictor, in both 
primal and dual spaces, and whilst the predictor points towards 
...
produce immediately the optimizer.
Given the system
\be
...
\[
  \hat x_i s_i + \hat s_i x_i \le n x_i s_i, \qquad i = 1, \ldots, n,
\]
which, summing up all terms, produces
...
propose to search a subspace spanned by $k$ different 
...
produces a search direction $\Delta w$ 
    ...
    \begin{algorithmic}[0]  \label{alg:WeightedCorrectors}
      ...
      \REPEAT
        ...
        \STATE Do a linesearch to find the optimal $\hat\omega$ that
               ...
               $\Delta^\omega = \Delta_a +\omega\Delta_c$
        \smallskip
	\STATE Set $\Delta_p := \Delta_p +\hat\omega\Delta_m$. 
        \smallskip
	   ...
	   \STATE Perform a linesearch to find the optimal $\hat\omega$ that
                  maximizes the stepsize $\alpha$ in
                  $\Delta^\omega = \Delta_p +\omega\Delta_m$.
           \smallskip
           \STATE Set $\Delta_p := \Delta_p +\hat\omega\Delta_m$.
           \smallskip
        \ENDWHILE
...
Therefore, optimal termination occurs when the conditions
