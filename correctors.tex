...
interior point implementations. In particular, as was noted by
...
\fb{
Run netlib problems with HOPDM, print out Mehrotra rejected, and count them.
}
...
\item[Repeat] for $k=0,1,2,\ldots$ until some convergence criteria are met:
  ...
  \item Set $\mu$ according to (\ref{eq:Mu}) and find Mehrothra's corrector 
...
This neighbourhood was implicitly used in the previous section 
...
The analysis is done for the long-step feasible path-following algorithm 
and follows closely the presentation of \cite[Chapter~5]{ipm:Wright97}. 
%
In this context, the search direction $(\Delta x, \Delta y, \Delta s)$ 
...
Our main result is presented in Lemma~\ref{SymLemma}. We prove that 
it is possible to find a strictly positive stepsize $\alpha$ 
such that the new iterate 
$(x(\alpha),y(\alpha),s(\alpha))=(x,y,s)+\alpha(\Delta x, \Delta y, \Delta s)$
will not leave the symmetric neighbourhood, 
and thus this neighbourhood is well defined. This result extends 
...
With this theoretical guidance, in the next section we will proceed 
