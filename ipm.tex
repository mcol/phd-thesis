...
{\em long-step algorithms}) are less conservative their short-step
...
a complete path-following algorithm. We then discuss various
...
in (\ref{eq:NewtonSystem}). It is easy to see that feasibility
...
From (\ref{eq:OptimalityProgress}) we can make some observations.
The length of the step $\alpha$ taken in the search direction
$(\Delta x,\Delta y ,\Delta s)$ computed from (\ref{eq:NewtonSystem})
controls the reduction in complementarity gap: for the chosen
$\sigma$, the longer the step, the bigger the reduction. This motivates
...
Mizuno, Todd and Ye \cite{MizunoToddYe}, who analysed a short-step 
...
to a short-step feasible method, also the Mizuno-Todd-Ye
predictor--corrector algorithm \cite{MizunoToddYe}
converges in $\bigO(\sqrt{n}L)$ iterations, with the only
