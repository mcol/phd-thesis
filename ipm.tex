...
Complementarity can be seen as a certificate for optimality \cite{phd:Jansen}
and maybe even \cite{Schrijver86}.

For non-optimal solutions, complementarity measures the distance of the
solution to optimality. When this distance is driven to zero, then the
...
\fb{
Connect curves with changes in the optimal partition.
}
...
\fb{
The limit of $(P_\mu)$ for $\mu \to \infty$ determines the analytic 
center of the feasible region. As $\mu \to 0$, the solution to
$(P_\mu)$ converges to the analytic center of the set of optimal
solutions of $(P)$.
}
...
The choice between augmented system and normal equations is based on 
the relative density of $AD^2A^T$ with respect to $A$.

Normal equations are bad when there are dense columns in $A$, as they
generate dense blocks in $AD^2A^T$.
