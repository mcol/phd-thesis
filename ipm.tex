...
\be \label{eq:PrimalDualPair}
  \begin{array}{crlp{2cm}crl}
     & \displaystyle \min_x & c^T x & & & \displaystyle \max_{(y,s)} & b^T y \\
...
\[
\min_x \; c^T x \;\quad \mbox{s.t. }\;  x    \in \mathcal{P}; \qquad
\max_{(y,s)} \; b^T y \;\quad \mbox{s.t. }\; (y,s) \in \mathcal{D}.
\]
...
(see Section~\ref{sec:InfeasibleMethods}) or by introducing 
some controlled perturbations that enlarge the primal--dual 
feasible set \cite{CartisGould}.
...
\[
  (P_\mu) \quad \min_x\; c^Tx - \mu \displaystyle\sum_{i=1}^n \ln x_i
          \;\quad \mbox{s.t }\; x \in \mathcal{P}^0.
\]
Problem $(P_\mu)$ denotes a family of problems parameterised by the scalar
...
System (\ref{eq:PerturbedKKT}) determines 
a unique continuous smooth curve 
$w(\mu) = (x(\mu),y(\mu),s(\mu))$ towards the optimal set as $\mu\to 0$.
In interior-point 
...
\eqref{eq:AverageComplementarity}, together with the fact that
$c^Tx(\mu) \ge c^Tx^* = b^Ty^* \ge b^Ty(\mu)$, imply that as $\mu\to 0$
\[
   c^Tx(\mu)\to c^Tx^* \quad \text{ and } \quad b^Ty(\mu) \to b^Ty^*,
\]
so the objective function values for the perturbed
problem converge to those achieved by an optimal solution
$(x^*,y^*,s^*)$ of the original problem.
...
an optimal solution of problem (\ref{eq:PrimalDualPair}).
...
This is described in the following result \cite{ipm:Wright97,Ye97}.

\begin{theorem}[Strict complementarity]  \label{th:StrictComplementarity}
...
\]
From Theorem~\ref{th:StrictComplementarity} it follows that
...
solution rather than at the analytic center of the optimal face.
...
can be defined, but they are usually too demanding to compute
\cite{Gonzaga92}. 
...
guarantees
\be  \label{eq:FeasibleSearchDirection}
...
\[
  ...
  \end{split}
\]
Moreover, using \eqref{eq:FeasibleSearchDirection} we obtain
\be  \label{eq:DeltaxDeltas}
...
\]
where we used \eqref{eq:DeltaxDeltas} and the fact that
...
with $\sigma = 1$ is usually called a {\em pure centering step}.
...
\]
independent of $n$. 
This is an aggressive update, and a full Newton
...
in general, a nontrivial task, as solving the feasibility problem
is an optimization problem in its own right.
...
When feasibility is restored, that is when $\xi_b = \xi_c = 0$,
an infeasible algorithm becomes identical
...
based on it are extremely conservative and, in practice, very slow.
...
is not adequately represented in a computational setting by either 
