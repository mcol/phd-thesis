...
%Mar:  8,  9, 13, 15, 16, 17, 19, 20, 21, 22, 27
...
the following holds:
\[
  c^Tx - b^Ty = c^Tx - x^TA^Ty = x^T(c - A^T y) = x^Ts \ge 0.
\]
\end{proof}
...
\[
\begin{array}{cl}
 (P_\mu)\quad & \min\; c^Tx - \mu \sum_i \ln x_i, \quad x \in \mathcal{P}^0.
\end{array}
\]
...
of redundant constraints. Here we mention the fact that presolve
