...
It is common to consider {\em balanced trees}, in which the number of
branches leaving from each node is the same at all stages.
An alternative approach, preferred in multi-stage models of very
large dimensions, is to consider more branches in the
earlier stages than in the later ones, giving rise to the
so-called {\em left-heavy trees}.
The framework we introduce in Section~\ref{sec:ReducedTree} can deal
with any shape of trees.
A very simple event tree, which we will use later on for illustration,
is shown in Figure~\ref{fig:EventTree}.
%
\begin{figure}[ht]
  \begin{center}
    \includegraphics[scale=0.6]{figures/tree.eps}
    \caption{An event tree.}
    \label{fig:EventTree}
  \end{center}
  \vspace{-3ex}
\end{figure}
...
\begin{array}{rl}
  \displaystyle \min_{x^1} & \displaystyle (q^1)^T x^1 
                         + \E_\xi\big[\min_{y(\xi)}\, q(\xi)^T y(\xi)\big] \\
  \text{s.t.} &  W^1x^1                   = h^1,      \\
...
\be  \label{eq:CompleteProblem}
  \min_x\; c^T x \;\quad \mbox{s.t. }\; Ax = b, \;\; x \ge 0,
\ee
...
\min_{x_R}\; c_R^T x_R \;\quad \mbox{s.t. }\; A_R x_R = b_R, \; x_R \ge 0,
\ee
...
However, similarly to what we did with the solution vector, 
...
\min_x\; \hat{c}^T x \;\quad \mbox{s.t. }\; \hat{A} x = \hat{b},
    \; x \ge 0,
\ee
...
where
\be  \label{eq:rho}
  \rho = \min_{i\in\Ctree}\left\{\frac{p^i}{p_R^{r(i)}}\frac{n}{n_R}, 
...
After some straightforward manipulations following the arguments of 
...
%
It should be noted that we disabled \HOPDM's presolve % and scaling 
...
The problems solved show an overall good behaviour of our warm-start
...
\begin{itemize}
\item The reduced tree approach to construct a warm-start iterate 
