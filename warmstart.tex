...
We introduce the set $\mathcal{I}_k$ of nodes in the complete tree 
that are initialised by a given node $k\in\Rtree$, that is
...
probabilities. Denote by $\delta^i := p^i/p^{a(i)}$ the probability of
reaching node $i$, given that its parent $a(i)$ has already been
reached. For a node $l$ in the $k$-th
...
\]
The upper bound showing $\hat x^l_j\hat s^l_j\le \hat{\mu}/\rho$
