...
\begin{equation}  \label{DetEquiv}
  ...
  \mbox{s.t.} & W^1 x^1 &\!\!\!\! = h^1, \\
    & T^{l_t} x^{a(l_t)} + W^{l_t} x^{l_t} &\!\!\!\! = h^{l_t}, 
                               &\quad l_t\in\mathcal{L}_t,\; t = 2,\dots,T, \\
    & x^{l_t} &\!\!\!\!\!\!\! \geq 0,
                               &\quad l_t\in\mathcal{L}_t,\; t = 1,\dots,T. \\
  \end{array}
\end{equation}
...
only over the descendants of $a(l)$, otherwise $\delta_R^l$ may be 
bigger than 1.
...
\ee
%
As $(x,y,s)\in \Nhood_s(\gamma)$, using Lemma~\ref{th:BoundQ} we get
