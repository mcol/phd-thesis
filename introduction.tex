...
that discovers an optimal vertex among the multitude of non-optimal ones.
...
%of the multiple centrality correctors technique \cite{Gondzio96}.
...
Moreover, it is unrealistic to expect that a corrector direction $\Delta_c$
can absorb the error made by a predictor direction $\Delta_p$;
however, this direction can be used to find a new composite
direction $\Delta^\omega = \Delta_p + \omega\Delta_c$, where
$\omega \in [0,1]$.
